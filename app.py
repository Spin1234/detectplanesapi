...
@app.route("/planes")
def get_planes():
    # Get parameters from URL: ?lat=xx&lon=xx&range=xx
    lat = request.args.get("lat")
    lon = request.args.get("lon")
    dist = request.args.get("range", default="50") # Default to 50nm if not provided

    if not lat or not lon:
        return jsonify({"error": "Missing lat or lon parameters"}), 400

    try:
        # Construct the ADSB.lol URL directly
        target = f"https://api.adsb.lol/v2/lat/{lat}/lon/{lon}/dist/{dist}"
        
        # User-Agent is good practice
        headers = {'User-Agent': 'PlaneTrackerApp/1.0'}
